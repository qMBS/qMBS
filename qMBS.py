...
def Bicons(QuantumCircuit):
    circuit = QuantumCircuit
    qubit_indices = [0, 1, 2, 3]
    operations = [
        (circuit.x, [qubit_indices[2]]),
        (circuit.x, [qubit_indices[0]]),
        (circuit.cx, [qubit_indices[0], qubit_indices[3]]),
        (circuit.x, [qubit_indices[0]]),
    ]
    for operation, qubits in operations:
        operation(*qubits)

def BiconsInv(QuantumCircuit):
    circuit = QuantumCircuit
    qubit_indices = [0, 1, 2, 3]
    operations = [
        (circuit.x, [qubit_indices[0]]),  
    ...
    for operation, qubits in reversed(operations):
        operation(*qubits)
